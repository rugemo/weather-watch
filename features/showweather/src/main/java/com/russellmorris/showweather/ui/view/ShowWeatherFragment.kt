    ...
    private fun update(resource: Resource<Weather>?) {
        resource?.let {
            it.data?.let {
                ...
                showCityAndCountryInfo()
            }
            it.message?.let { snackBar.show() }
        }
    }

    private fun showCityAndCountryInfo() {
        if (args.city != null && args.country != null) {
            cityAndCountry.text = getString(R.string.city_country, args.city, args.country)
            cityAndCountry.visible()
        }
    }
